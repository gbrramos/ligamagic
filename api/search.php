...
// Se o token for válido, entra no IF
if ($token){
    // Seta a requisição vinda do front na variável produto
    $produto = addslashes($_GET['nome']);

    // Conta quantos registros existem no banco de dados e converte em indice
    ...
    if($count > 0){
        ...
        $values = array('R$ '.$preco, 'Última alteração: '.$update);
    }
    else{
        // Se o banco não encontrar nada
        $values = array('Produto não encontrado',';-;');
    }
